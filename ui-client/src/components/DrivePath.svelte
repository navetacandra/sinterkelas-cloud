<script>
  ...
  $: {
    ...
    if (paths.length > 4) {
      ...
      pathMid = paths.slice(1, paths.length - 2).reverse();
      pathBack = paths.slice(-2);
    } else {
      pathFront = paths;
    }
  }
  ...
  {#each pathBack as path, i}
    <p class="text-black dark:text-white flex items-center">
      ...
      {#if i < pathBack.length - 1}
        <Link
          class="mx-1 cursor-pointer text-blue-800 hover:text-blue-600 dark:text-blue-500 dark:hover:text-blue-400 hover:underline"
          to={`/drive/${path.id}`}>{path.name || "Home"}</Link
        >
      {:else}
        <span class="mx-1 cursor-default text-black">{path.name || "Home"}</span
        >
      {/if}
    </p>
  {/each}
